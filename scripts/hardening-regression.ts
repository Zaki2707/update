...
assert.match(server, /meta\?\.sanSignature === sanSignature/);
  ...
  assert.match(studentPdfBlock, /\/api\/exam\/my-review/);
  assert.equal(studentPdfBlock.includes('Kunci Jawaban'), false, 'student PDF must not expose answer keys');
  assert.equal(studentPdfBlock.includes('(Benar)'), false, 'student PDF must not label answers correct');
  assert.equal(studentPdfBlock.includes('(Salah)'), false, 'student PDF must not label answers incorrect');
