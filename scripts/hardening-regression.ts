...
assert.match(server, /meta\?\.sanSignature === sanSignature/);
  ...
  assert.match(server, /activeSeconds: Math\.max\(activeSeconds, Number\(existing\.activeSeconds \|\| 0\)\)/);
  assert.match(server, /viewedBlockIds: viewedBlockIds\.length > 0/);
  ...
  assert.equal(app.includes("endpoint = '/api/exam-monitoring-state'"), false, 'generic realtime must not pull the tenant-wide CBT monitoring snapshot');
  assert.match(app, /CBT_REALTIME_NO_GLOBAL_SNAPSHOT_V1/);
  ...
  assert.match(server, /app\.post\("\/api\/load-test\/cleanup-exam", requireAuth, requireRole\(\['admin', 'bos', 'superadmin'\]\)/);
  assert.match(server, /DELETE_LOAD_TEST_STATE/);
  assert.match(server, /function isDedicatedLoadTestExam\(/);
