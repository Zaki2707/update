...
const checks = [
  ...
  ['Teacher exam sync preserves unauthorized existing records', server.includes('function examsForRequest') && server.includes('allowedIncoming') && server.includes('Existing exams') === false && server.includes('outside the teacher\'s assignment are ignored and preserved')],
