...
const checks = [
  ...
  ['Auto-correction resolves exam, class, and LKPD inside tenant', server.includes('ID ujian ambigu lintas tenant. Pilih tenant target secara eksplisit.') && server.includes('ID LKPD ambigu lintas tenant. Pilih tenant target secara eksplisit.') && server.includes('Ujian dan kelas harus berasal dari tenant yang sama.') && server.includes('LKPD dan kelas harus berasal dari tenant yang sama.')],
  ...
  ['Atomic multi-key persistence uses a PostgreSQL transaction', server.includes('ATOMIC_BATCH_PERSISTENCE') && server.includes("await client.query('BEGIN')") && server.includes("await client.query('COMMIT')") && server.includes("await client.query('ROLLBACK')")],
  ['Token ledger critical mutations are copy-on-write', server.includes('const nextTokenRequests = tokenRequests.map') && server.includes('const nextMadrasahs = madrasahs.map') && server.includes('const nextUsedActivationKeys = [...usedActivationKeys, signature]')],
  ['Regular users only receive member-safe madrasah metadata', server.includes('function sanitizeMadrasahMemberView') && server.includes('ownMadrasahs.map(sanitizeMadrasahMemberView)')],
  ['Game rewards resist replay and oversized state', server.includes('rewardAlreadyClaimed') && server.includes('game-reward::') && server.includes('State sesi game terlalu besar.')],
  ['CBT student mutations validate attempt context and payload size', server.includes('Jawaban terlalu besar. Maksimal 64 KB per soal.') && server.includes('Format frame livecam tidak valid.') && server.includes('getExamAttemptContext(req, authUser, sId, eId)')],
  ['ChildGuard student sync is identity scoped in all modes', server.includes('Status ChildGuard hanya boleh untuk akun siswa sendiri.') && !server.includes('if (isOnlineMode && isStudentSyncRole)')],
  ['Teacher question-bank subject scope is server enforced', server.includes('QUESTION_BANK_TEACHER_SUBJECT_SCOPE') && server.includes('teacher-subject-merge') && server.includes('filterQuestionsForRequest(filteredQuestions, req)')],
  ['Student LKPD monitoring uses dedicated identity-scoped endpoint', server.includes('LKPD_STUDENT_STATE_V1') && server.includes('app.post("/api/lkpd/student-state"') && fs.readFileSync('src/lkpdModule.js', 'utf8').includes("fetch('/api/lkpd/student-state'")],
  ...
  ['Teacher exam monitor resolves tenant-safe exam identity', server.includes('const resolvedExam = resolveTenantItemIndexById(examSource, eId, req)')],
