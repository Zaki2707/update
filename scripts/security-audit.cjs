...
const checks = [
  ...
  ['Disabled LiveKit endpoint cannot bypass teacher signaling scope', server.includes('P2P_ONLY_LIVECAM_SERVER_V5') && server.includes('app.post("/api/exam/livekit-token", requireAuth') && server.includes('status(410)') && server.includes('teacherCanMonitorStudentRealtime(wsReq, user, target)')],
