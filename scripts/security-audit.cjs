...
const checks = [
  ...
  ['LiveKit requires explicit credentials in every mode', server.includes('LIVEKIT_EXPLICIT_CREDENTIALS_V3') && server.includes('if (!apiKey || !apiSecret || !serverUrl)') && server.includes('new AccessToken(apiKey, apiSecret') && !server.includes('apiKey || "devkey"') && !server.includes('apiSecret || "secret"') && !server.includes('serverUrl || "ws://localhost:7880"')],
  ['LiveKit capability is server-authoritative and secret-free', server.includes('LIVEKIT_RUNTIME_CAPABILITY_V4') && server.includes('resolveLiveKitRuntimeConfig().configured') && assessment.includes('LIVEKIT_CLIENT_CAPABILITY_V4') && assessment.includes('s.livekitConfigured === true') && assessment.includes('LIVEKIT_TO_P2P_FAILOVER_V4') && !assessment.includes("const key = s.livekitApiKey || ''")],
