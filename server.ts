Warning: truncated output (original token count: 241953)
Total output lines: 21114
...
function collectOfflineHttpsAltNames(): any[] {
  ...
  try {
    ...
    if (host) dnsNames.add(host);

    const ifaces = os.networkInterfaces();
    for (const entries of Object.values(ifaces)) {
      for (const iface of entries || []) {
        if (iface.family === 'IPv4' && !iface.internal && iface.address) {
          ipNames.add(String(iface.address));
        }
      }
    }
  } catch (_) {}

  String(process.env.OFFLINE_HTTPS_HOSTNAMES || '')
    .split(',')
    .map(v => v.trim())
    .filter(Boolean)
    .forEach(name => dnsNames.add(name));

  return [
    ...Array.from(dnsNames).map(value => ({ type: 2, value })),
    ...Array.from(ipNames).map(ip => ({ type: 7, ip }))
  ];
}
...
async function getOfflineHttpsRuntimeOptions(): Promise<OfflineHttpsRuntimeOptions | null> {
  ...
  const customCertPath = String(process.env.OFFLINE_HTTPS_CERT_PATH || '').trim();
  if (Boolean(customKeyPath) !== Boolean(customCertPath)) {
    throw new Error('OFFLINE_HTTPS_KEY_PATH dan OFFLINE_HTTPS_CERT_PATH harus diisi berpasangan.');
  }

  if (customKeyPath && customCertPath) {
    const keyPath = resolveRuntimePath(customKeyPath);
    const certPath = resolveRuntimePath(customCertPath);
    if (!fs.existsSync(keyPath) || !fs.existsSync(certPath)) {
      throw new Error('File sertifikat HTTPS offline tidak ditemukan. Periksa OFFLINE_HTTPS_KEY_PATH/OFFLINE_HTTPS_CERT_PATH.');
    }
    return {
      key: fs.readFileSync(keyPath),
      cert: fs.readFileSync(certPath),
...
function generateTemporaryStudentPassword(): string {
  // 12 URL-safe random characters. Returned once through the credentials payload.
...
// TEACHER_EVENT_MUTATION_SCOPE_V3: event containers remain readable so teacher
// schedules stay grouped correctly, but only admin/BOSS may mutate EVENT records.
function teacherCanMutateExamPayload(req: any, examPayload: any): boolean {
  if (!isTeacherRequest(req)) return true;
  if (!examPayload || typeof examPayload !== 'object') return false;
  if (String(examPayload.recordType || '').toUpperCase() === 'EVENT') return false;
  return teacherCanUseExamPayload(req, examPayload);
}
...
function classTargetsFromLearningMaterial(material: any): string[] {
  // When the multi-class fields exist, they are authoritative. Do not append
  // legacy classId/className values, which may refer to a class removed while
  // editing the material and would leak it to the old class again.
...
app.post("/api/students", requireAuth, requireRole(['admin', 'bos', 'superadmin']), async (req, res) => {
...
app.post("/api/students/import", requireAuth, requireRole(['admin', 'bos', 'superadmin']), async (req, res) => {
  ...
  for (const item of importedList) {
    ...
    const requestedPassword = String(item.password || '').trim();
    const rawPassword = requestedPassword.length >= 8 ? requestedPassword : generateTemporaryStudentPassword();
    const hashed = hashPassword(rawPassword);
...
app.post("/api/students/bulk-upload-photos", requireAuth, requireRole(['admin', 'bos', 'superadmin']), async (req, res) => {
...
app.put("/api/student/profile", requireAuth, requireRole(['student', 'siswa', 'class_leader', 'ketua_kelas']), async (req: any, res) => {
  ...
  if (password && (password.length < 8 || password.length > 128)) {
    return res.status(400).json({ success: false, message: "Password baru harus 8-128 karakter." });
  }
...
app.put("/api/students/:id", requireAuth, requireRole(['admin', 'bos', 'superadmin']), async (req, res) => {
  const { id } = req.params;
...
app.post("/api/students/:id/set-profile-photo", requireAuth, requireRole(['student', 'siswa', 'class_leader', 'ketua_kelas', 'teacher', 'guru', 'admin', 'bos', 'superadmin']), async (req: any, res) => {
  const { id } = req.params;
  const photoUser = req.user || getAuthUser(req);
  if (isStudentAuthRole(String(photoUser?.role || '').toLowerCase()) && String(photoUser?.id || '') !== String(id)) {
    return res.status(403).json({ success: false, message: "Siswa hanya dapat mengubah foto profil miliknya sendiri." });
  }
...
app.delete("/api/students/:id/photo-history", requireAuth, requireRole(['student', 'siswa', 'class_leader', 'ketua_kelas', 'teacher', 'guru', 'admin', 'bos', 'superadmin']), async (req: any, res) => {
  const { id } = req.params;
  const photoUser = req.user || getAuthUser(req);
  if (isStudentAuthRole(String(photoUser?.role || '').toLowerCase()) && String(photoUser?.id || '') !== String(id)) {
    return res.status(403).json({ success: false, message: "Siswa hanya dapat menghapus riwayat foto miliknya sendiri." });
  }
  ...
  if (db) {
    ...
    for (const rec of studentAttRecords) {
      if (rec && rec.photo && rec.photo.startsWith('/api/photos/')) {
        ...
        if (docId) {
          try { await deleteDoc(doc(db…41953 tokens truncated…s') || exams || [];
...
app.post("/api/exam-monitoring-state", requireAuth, requireRole(['teacher', 'guru', 'admin', 'bos', 'superadmin']), async (req: any, res) => {
  ...
  if (livecamFrame !== undefined && livecamFrame !== null) {
    if (!livecamFrame || typeof livecamFrame !== 'object' || Array.isArray(livecamFrame) || !livecamFrame.key || !livecamFrame.frame) {
      return res.status(400).json({ success: false, message: "Payload frame livecam monitoring tidak valid." });
    }
    ...
    const frameText = String(livecamFrame.frame);
    ...
    // LEGACY_LIVECAM_FRAME_VALIDATION_V3
    livecamFrame = { ...livecamFrame, key: normalizedLivecamKey, frame: frameText };
  }
...
// SIGNALING_TRANSPORT_BRIDGE_V4: WebSocket and HTTP polling are interchangeable
// transports. Prefer an active socket, otherwise queue the signal for the HTTP poller.
function sendSignalToSocket(targetWs: any, senderId: string, signal: any): boolean {
  if (!targetWs || targetWs.readyState !== 1) return false;
  try {
    targetWs.send(JSON.stringify({ type: 'signal', senderId, signal }));
    return true;
  } catch (_) {
    return false;
  }
...
function findActiveStaffSocket(tenant: string, staffId: string): any | null {
  ...
  if (direct?.readyState === 1) return direct;

  // BOSS/superadmin sockets are normally registered under their own default tenant.
  for (const candidateWs of wsClients.values()) {
    const candidateUser = candidateWs?.__authUser;
    const candidateRole = String(candidateUser?.role || '').toLowerCase();
    if (candidateWs?.readyState === 1 &&
        String(candidateUser?.id || '') === String(staffId) &&
...
function deliverSignalToStudent(tenant: string, studentId: string, senderId: string, signal: any): 'ws' | 'http' {
  const canonicalTenant = canonicalRealtimeTenant(tenant);
  const socketKey = 'student::' + canonicalTenant + '::' + String(studentId);
  if (sendSignalToSocket(wsClients.get(socketKey), senderId, signal)) return 'ws';
  enqueueExamSignal(socketKey, senderId, signal);
  return 'http';
}

function deliverSignalToStaff(tenant: string, staffId: string, senderId: string, signal: any): 'ws' | 'http' | 'none' {
  const canonicalTenant = canonicalRealtimeTenant(tenant);
  const staffSocket = findActiveStaffSocket(canonicalTenant, String(staffId));
  if (sendSignalToSocket(staffSocket, senderId, signal)) return 'ws';

  pruneSignalingStaffRoutes();
  const routeKey = signalingStaffKeyForTenant(canonicalTenant, staffId);
  const route = signalingStaffRoutes.get(routeKey);
...
function signalingStaffCandidatesForTenant(tenant: string): any[] {
  pruneSignalingStaffRoutes();
  const canonicalTenant = canonicalRealtimeTenant(tenant);
  const prefix = 'staff::' + canonicalTenant + '::';
  const byId = new Map<string, any>();

  for (const [key, socket] of wsClients.entries()) {
    if (!key.startsWith(prefix) || socket?.readyState !== 1) continue;
    const user = socket.__authUser;
    if (user?.id !== undefined && user?.id !== null) byId.set(String(user.id), user);
  }
  for (const [key, route] of signalingStaffRoutes.entries()) {
    if (!key.startsWith(prefix) || !route?.user) continue;
    byId.set(String(route.user.id), route.user);
  }
...
app.post("/api/exam/signaling", requireAuth, (req: any, res) => {
  ...
  if (student) {
    ...
    if (recipientId === 'admin') {
      let delivered = 0;
      for (const monitorUser of signalingStaffCandidatesForTenant(tenant)) {
        ...
        if (deliverSignalToStaff(tenant, String(monitorUser.id), senderId, signal) !== 'none') delivered++;
      }
      return res.json({ success: true, delivered });
    }

    const targetWs = findActiveStaffSocket(tenant, recipientId);
    const route = signalingStaffRoutes.get(signalingStaffKeyForTenant(tenant, recipientId));
    const monitorUser = targetWs?.__authUser || route?.user;
    if (!monitorUser) return res.status(404).json({ success: false, message: "Pengawas tujuan tidak aktif." });
    const monitorRole = String(monitorUser?.role || '').toLowerCase();
    ...
    const delivered = deliverSignalToStaff(tenant, recipientId, senderId, signal);
    return delivered === 'none'
      ? res.status(404).json({ success: false, message: "Pengawas tujuan tidak aktif." })
      : res.json({ success: true, transport: delivered });
  }
  ...
  const transport = deliverSignalToStudent(signalingItemTenant(target), recipientId, senderId, signal);
  ...
  if (Array.isArray(req.body)) {
    if (isTeacherRequest(req)) {
      ...
      for (const item of req.body) {
        ...
        if (teacherCanMutateExamPayload(req, item)) {
    ...
    if (isTeacherRequest(req) && !teacherCanMutateExamPayload(req, req.body)) {
    ...
    const idx = resolved.index;
    if (idx >= 0) {
      if (isTeacherRequest(req) && !teacherCanMutateExamPayload(req, exams[idx])) {
        return res.status(403).json({ success: false, message: "Guru tidak dapat mengubah event atau ujian di luar mata pelajaran yang diampu." });
      }
...
app.get("/api/settings", (req, res) => {
  ...
  if (safeSettings && typeof safeSettings === 'object') {
    safeSettings.livekitConfigured = resolveLiveKitRuntimeConfig().configured;
  }
  res.setHeader('Cache-Control', 'no-store');
...
async function processSystemRestore(backup: any) {
    ...
    if (rStudents) {
      ...
      for (const restoredStudent of mergedStudents) {
        const restoredPassword = String(restoredStudent?.password || '');
        const isHash = restoredPassword.startsWith('scrypt$') || restoredPassword.startsWith('sha256$');
        if (restoredPassword && !isHash) restoredStudent.password = hashPassword(restoredPassword);
        if (restoredStudent && typeof restoredStudent === 'object') delete restoredStudent.passwordRaw;
      }
    ...
    if (rTeachers) {
      ...
      for (const restoredTeacher of mergedTeachers) {
        const restoredPassword = String(restoredTeacher?.password || '');
        const isHash = restoredPassword.startsWith('scrypt$') || restoredPassword.startsWith('sha256$');
        if (restoredPassword && !isHash) restoredTeacher.password = hashPassword(restoredPassword);
        if (restoredTeacher && typeof restoredTeacher === 'object') delete restoredTeacher.passwordRaw;
      }
...
async function startServer() {
  ...
  const offlineHttpsOptions = await getOfflineHttpsRuntimeOptions();
  const localProtocol = offlineHttpsOptions ? 'https' : 'http';

  const onServerListening = () => {
    console.log(`\n======================================================`);
    console.log(`  MADRASAH BISA CBT & MANAJEMEN SERVER SIAP`);
    console.log(`======================================================`);
    console.log(`  > Akses Lokal Laptop : ${localProtocol}://localhost:${PORT}`);
      ...
      for (const name of Object.keys(ifaces)) {
        for (const iface of ifaces[name] || []) {
          if (iface.family === 'IPv4' && !iface.internal) {
            console.log(`  > Akses HP Siswa (LAN/Wi-Fi): ${localProtocol}://${iface.address}:${PORT}`);
            hasLan = true;
          }
        }
      }
    ...
    if (!hasLan) {
      console.log(`  > Akses Jaringan: ${localProtocol}://0.0.0.0:${PORT}`);
    }
  ...
  // Online/Cloud Run intentionally keeps the existing HTTP listener because TLS is
  // terminated by Cloud Run. HTTPS is created only for explicitly-enabled OFFLINE runtime.
  const server = offlineHttpsOptions
    ? https.createServer({ key: offlineHttpsOptions.key, cert: offlineHttpsOptions.cert }, app)
        .listen(PORT, "0.0.0.0", 1024, onServerListening)
    : app.listen(PORT, "0.0.0.0", 1024, onServerListening);
      ...
      ws.on("message", (message: any) => {
        try {
          ...
          if (data.type === "register") {
            ...
            if (student) {
              ...
              if (recipient === 'admin') {
                for (const monitorUser of signalingStaffCandidatesForTenant(tenant)) {
                  ...
                  deliverSignalToStaff(tenant, String(monitorUser.id), publicId, data.signal);
                }
                return;
              }

              const targetWs = findActiveStaffSocket(tenant, recipient);
              const targetRoute = signalingStaffRoutes.get(signalingStaffKeyForTenant(tenant, recipient));
              const monitorUser = targetWs?.__authUser || targetRoute?.user;
              const monitorRole = String(monitorUser?.role || '').toLowerCase();
              if (!monitorUser || !isStaffAuthRole(monitorRole)) return;
              if (monitorRole === 'teacher' || monitorRole === 'guru') {
                const monitorReq: any = { user: monitorUser, headers: {}, query: {}, body: {} };
                if (!teacherCanMonitorStudentRealtime(monitorReq, monitorUser, senderStudent)) return;
              }
              deliverSignalToStaff(tenant, recipient, publicId, data.signal);
              return;
            }
            ...
            rememberSignalingStaffRoute(user, targetTenant);
            deliverSignalToStudent(targetTenant, recipient, publicId, data.signal);
          }
