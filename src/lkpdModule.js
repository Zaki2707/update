...
function lkpdEscapeAttr(value) {
    return lkpdEscapeHtml(value);
}

function lkpdEncodedArg(value) {
...
function lkpdSafeImageSrc(value) {
    ...
    if (!raw) return '';
    if (window.isSafeDisplayImageUrl && window.isSafeDisplayImageUrl(raw)) return raw;
                            ...
                            <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm hover:shadow-md transition overflow-hidden flex flex-col justify-between group">
                                <div class="p-6 space-y-4">
                                    ...
                                    <div class="h-32 rounded-2xl bg-slate-900 overflow-hidden relative border border-slate-200 shadow-inner flex items-center justify-center cursor-pointer" onclick="openManageLkpdModal('${lk.id}')" title="Klik untuk mengelola lembar kerja">
                                        ${lkpdSafeImageSrc(lk.customImage) ? `
                                            <img src="${lkpdEscapeAttr(lkpdSafeImageSrc(lk.customImage))}" alt="Custom LKPD" class="w-full h-full object-cover" />
                                        ` : `
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-2xl max-w-lg w-full overflow-hidden my-4 sm:my-8 flex flex-col max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-4rem)]">
                ...
                <form onsubmit="handleSaveLkpdForm(event, '${lkpdId || ''}')" class="p-6 space-y-4 text-xs overflow-y-auto flex-1 custom-scrollbar">
                        ...
                        <div class="relative">
                            <select id="lkpd-form-subject" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-800 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                                ...
                                ${subjects.map(s => {
                                    ...
                                    return `<option value="${lkpdEscapeAttr(sName)}" ${isSel ? 'selected' : ''}>${lkpdEscapeHtml(sName)}</option>`;
                                }).join('')}
                    ...
                    <div>
                        ...
                        <select id="lkpd-form-class" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-800 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                            ...
                            ${classes.map(c => {
                                ...
                                return `<option value="${lkpdEscapeAttr(cId)}::${lkpdEscapeAttr(cName)}" ${isSel ? 'selected' : ''}>${lkpdEscapeHtml(cName)}</option>`;
                            }).join('')}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="lkpd-form-date" required value="${lkpdEscapeAttr(lkpd && lkpd.date ? lkpd.date : '')}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-800 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="lkpd-form-desc" rows="2" placeholder="Tuliskan petunjuk pengerjaan lembar kerja..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-700 focus:ring-2 focus:ring-emerald-500 focus:outline-none">${lkpdEscapeHtml(lkpd ? (lkpd.description || '') : '')}</textarea>
                    </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-md w-full overflow-hidden my-6">
                ...
                <form onsubmit="handleSaveMarkerForm(event, '${lkpd.id}', '${marker.id}')" class="p-5 space-y-4 text-xs font-semibold text-slate-700">
                    ...
                    <div>
                        ...
                        <textarea id="mk-q" rows="3" required class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl">${lkpdEscapeHtml(marker.question || '')}</textarea>
                    </div>

                    <div>
                        <label class="block mb-1">Kunci Jawaban Guru / Rubrik Penskoran</label>
                        <textarea id="mk-ans" rows="2" placeholder="Contoh: Mitokondria (penghasil energi ATP)" class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl">${lkpdEscapeHtml(marker.answerKey || '')}</textarea>
                    </div>
                ...
                <div class="grid grid-cols-1 ${lkpd.answeringMode === 'on-photo' ? '' : 'lg:grid-cols-12'} gap-6 p-4 sm:p-6 flex-1">
                    ...
                    <div class="lg:col-span-5 flex flex-col bg-slate-50 border border-slate-200 rounded-3xl shadow-inner lg:sticky lg:top-4 lg:h-[calc(100vh-3rem)]">
                                    ...
                                    <div id="answer-card-${mk.id}" class="bg-white p-4 rounded-2xl border border-slate-200 transition focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 shadow-sm space-y-2">
                                        ...
                                        <div class="pt-1">
                                            ...
                                            ${mk.responseType === 'essay' ? `
                                                <textarea id="ans-input-${mk.id}" rows="3" placeholder="Tuliskan uraian jawaban Anda..." class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-slate-800 transition disabled:opacity-75 disabled:bg-slate-100 disabled:cursor-not-allowed" ${existingSub ? 'disabled' : ''}>${lkpdEscapeHtml(curVal)}</textarea>
                                            ` : `
                                                <input type="text" id="ans-input-${mk.id}" value="${lkpdEscapeAttr(curVal)}" placeholder="Tuliskan nama / isian singkat..." class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-slate-800 transition disabled:opacity-75 disabled:bg-slate-100 disabled:cursor-not-allowed" ${existingSub ? 'disabled' : ''}>
                                            `}
                                        </div>
                                    </div>
                                `;
                        ...
                        <div class="p-4 border-t border-slate-200 bg-white shrink-0 space-y-2 shadow-sm rounded-b-3xl">
                            ...
                            ` : `
                                <button type="button" onclick="submitStudentLkpdAnswers(decodeURIComponent('${lkpdEncodedArg(lkpd.id)}'), decodeURIComponent('${lkpdEncodedArg(stId)}'), decodeURIComponent('${lkpdEncodedArg(stName)}'), decodeURIComponent('${lkpdEncodedArg(stNis)}'))" class="w-full py-3 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-extrabold text-xs rounded-2xl shadow-lg shadow-emerald-600/20 transition flex items-center justify-center gap-2 cursor-pointer">
                ...
                <div class="p-6 bg-slate-50 border-t border-slate-200 rounded-b-3xl flex justify-center">
                     <div class="max-w-md w-full">
                          ${existingSub ? `
                            ...
                            ` : `
                                <button type="button" onclick="submitStudentLkpdAnswers(decodeURIComponent('${lkpdEncodedArg(lkpd.id)}'), decodeURIComponent('${lkpdEncodedArg(stId)}'), decodeURIComponent('${lkpdEncodedArg(stName)}'), decodeURIComponent('${lkpdEncodedArg(stNis)}'))" class="w-full py-4 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-extrabold text-sm rounded-2xl shadow-lg shadow-emerald-600/20 transition flex items-center justify-center gap-2 cursor-pointer">
                                    <i class="fa-solid fa-paper-plane"></i> <span>Kumpulkan Jawaban LKPD</span>
                                </button>
                            `}
                     </div>
            ...
            <div class="bg-white/90 backdrop-blur-md rounded-3xl border border-white/50 shadow-2xl max-w-sm w-full p-6 space-y-4 animate-scale-up" onclick="event.stopPropagation()">
                ...
                <div class="space-y-2">
                    ...
                    <div class="pt-2">
                        ...
                        ${mk.responseType === 'essay' ? `
                            <textarea id="temp-ans-input" rows="4" placeholder="Ketik jawaban lengkap di sini..." class="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-2xl text-xs font-bold focus:outline-none focus:border-indigo-500 transition shadow-inner disabled:opacity-60" ${existingSub ? 'disabled' : ''}>${lkpdEscapeHtml(curVal)}</textarea>
                        ` : `
                            <input type="text" id="temp-ans-input" value="${lkpdEscapeAttr(curVal)}" placeholder="Jawaban singkat..." class="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-2xl text-xs font-bold focus:outline-none focus:border-indigo-500 transition shadow-inner disabled:opacity-60" ${existingSub ? 'disabled' : ''}>
                        `}
                    </div>
                </div>
                    ...
                    ${lkpdStudents.map(st => {
                        ...
                        return `
                            <div class="group relative bg-slate-900 border ${isBlocked ? 'border-rose-500 ring-2 ring-rose-500/20' : isOutOfTab ? 'border-amber-500 animate-pulse' : 'border-slate-800'} rounded-3xl shadow-xl overflow-hidden flex flex-col justify-between hover:scale-[1.01] transition-all duration-300">
                                ...
                                <div onclick="focusStudentLivecam(decodeURIComponent('${lkpdEncodedArg(st.id)}'))" title="Klik untuk membuka Spotlight Livecam HD ${lkpdEscapeAttr(st.name)}" class="relative w-full aspect-video bg-slate-950 overflow-hidden cursor-pointer group/media">
                                ...
                                <div class="p-4 space-y-3.5 bg-slate-900 border-t border-slate-800">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button type="button" onclick="event.stopPropagation(); openSendLkpdMessageModal(decodeURIComponent('${lkpdEncodedArg(st.id)}'), decodeURIComponent('${lkpdEncodedArg(st.name)}'), decodeURIComponent('${lkpdEncodedArg(activeLkpd.id)}'))" class="px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[10px] rounded-xl transition flex items-center justify-center gap-1 cursor-pointer">
                                        ...
                                        ${filteredSubmissions.map(sub => {
                                            return `
                                                <tr class="hover:bg-slate-50/80 transition">
                                                    ...
                                                    <td class="py-4 px-4 text-slate-500">${lkpdEscapeHtml(sub.nis || '-')}</td>
...
window.cetakNilaiLkpdEvaluasi = function(classId, lkpdId) {
    ...
    const htmlContent = `
        ...
        <head>
            <meta charset="utf-8">
            <title>Laporan_Nilai_LKPD_${lkpd.title.replace(/\s+/g, '_')}</title>
            <style>
